...
control_user_led1
    STMIA sp!, {R4, LR}             ; Store R4 and the Link Register on stack
    
    MOV R2, #GPIOA_BASE             ; Load GPIO A base register address
    LDR R4, [R2, #GPIOA_ODR]        ; Read current ODR register value

    CMP R0, #1                      ; Check value of state variable
    ITE EQ
    ORREQ.W R4, R4, #GPIOA_BIT_5    ; Set the 
    BICNE.W R4, R4, #GPIOA_BIT_5
    
    STR.W R4, [R2, #GPIOA_ODR]
      
    MOV.W R0, R1                    ; Setup parameter for Delay function
